...
sudo sed -i "s/$  ${dollar_symbol}codenvy_admin_initial_password.*=.*/  ${dollar_symbol}codenvy_admin_initial_password = \"${OLD_system_ldap_password}\"/" "${PATH_TO_MANIFEST}" &>> $LOG_FILE
